...
class Usr extends CI_Model {
    ...
    public function selectUsrsLodging() {
     ...
     public function selectUsrsCount() {
        $query = $this->db->query("SELECT count(*) cont FROM usr");
        return($query->row());
    }
    ...
    public function verifyEmail($email) {
        ...
        $query = $this->db->query("SELECT email FROM usr WHERE email ='" . $email . "'");
        if ($query->num_rows() > 0) {
            $retorno = false;
        }
    ...
    public function searchDisponibility($sex) {
        ...
        $query = $this->db->query("SELECT (d.quantity-cons2.cont) loqqueda, h.idHost, d.sex, h.name, h.surname, h.telephone, d.quantity, cons2.cont
                                    ...
                                    INNER JOIN disponibility d ON d.idHost=h.idHost AND d.sex=cons2.sex AND d.year='2018' AND d.sex='".$sex."' 
                                    HAVING loqqueda>0");
        return($query->result());
...
public function reportForCity($idCity){
    $query = $this->db->query("SELECT surname, name, sex, birthday, telephone, email 
                                FROM usr   
                                WHERE idCity='".$idCity."' ORDER BY surname, name");
    //var_dump($idCity);
   // echo $idCity;
    return($query->result());
}
